...
class ising:
    ...
    def __init__(self, netsize, Nsensors=1, Nmotors=1):  # Create ising model
        ...
        self.env.max_speed = 8
        self.env.max_torque = 2.
        self.env.dt = .05
        ...
        # Pendulum-v0 can take as action every real value between continuous interval [-max_torque, max_torque]
        # which indicates the torque force to apply.
        # So we need to discretize this interval and take a number of values that motor neurons can represent.
        self.possibleActions = np.linspace(-self.env.max_torque, self.env.max_torque, num=2 ** self.Msize)
        self.Update(0)
    ...
    def Move(self):
        action = self.possibleActions[bool2int((self.s[-self.Msize:] + 1) // 2)]
        observation, reward, done, info = self.env.step([action])

    def UpdateSensors(self):
        x = np.sin(self.env.env.state[0])
        y = np.cos(self.env.env.state[0])

        self.s[0:self.Ssize1] = 2 * bitfield(np.digitize(x, self.sensorbins) - 1, self.Ssize1) - 1
        self.s[self.Ssize1:self.Ssize] = 2 * bitfield(np.digitize(y, self.sensorbins) - 1, self.Ssize1) - 1
    ...
    def deltaE(self, i):
        return 2 * (
            self.s[i] * self.h[i] + np.sum(self.s[i] * (self.J[i, :] * self.s) + self.s[i] * (self.J[:, i] * self.s)))
    ...
    def HomeostaticGradient(self, T=None):
        ...
        for t in range(T):
            ...
            self.SequentialUpdate()
            self.theta[t] = self.env.env.state[0]
            self.m += self.s
            for i in range(self.size):
    ...
    def CriticalLearning(self, Iterations, T=None):
        ...
        print(count, fit, np.max(np.abs(self.J)), np.mean(self.theta), np.max(self.theta))
        self.l2 = 0.004
